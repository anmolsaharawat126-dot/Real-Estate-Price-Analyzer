...
.stTextInput>div>div>input{
    background:#0e1219 !important;color:#e8eaf0 !important;
    border:1px solid rgba(245,166,35,0.3) !important;
    border-radius:10px !important;padding:0.6rem 1rem !important;font-size:1rem !important;}
.stTextInput>div>div>input::placeholder{color:#4a5568 !important;}
.stTextInput label{color:#e8eaf0 !important;font-weight:500 !important;}
</style>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════
# ACCURATE 2024 INDIAN REAL ESTATE RATES (₹/sqft)
# Source: NHB, Magicbricks, 99acres, PropEquity data
# ══════════════════════════════════════════════════════════════
CITY_BASE_RATES = {
    # TIER 1 METROS
    ...
    # UP / TIER 3 CITIES (Accurate 2024 rates)
    ...
    "etawah":1900,        "shahjahanpur":2000,"rampur":1800,
    "hapur":2800,         "bulandshahr":2600, "amroha":2000,
    "sambhal":1900,       "bijnor":1900,      "shamli":2000,
    "khatauli":2200,      "jhansi":2200,      "gorakhpur":2800,
    "varanasi":3200,      "prayagraj":3000,
    # DEFAULT
    "default_urban":2200, "default_rural":900,
}

# ══ AREA MULTIPLIERS (only for specific premium localities) ══
# These should only apply when area is EXPLICITLY matched
AREA_MULTIPLIERS = {
    # Mumbai
    "malabar hill":3.2,   "cuffe parade":3.0,   "worli":2.5,
    "bandra":2.2,         "juhu":2.0,            "powai":1.4,
    "andheri":1.1,        "borivali":0.9,        "lower parel":2.0,
    # Delhi
    "lutyens":3.5,        "defence colony":2.8,  "vasant vihar":2.5,
    "hauz khas":2.2,      "lajpat nagar":1.6,    "dwarka":0.9,
    "rohini":0.85,        "saket":1.8,           "greater kailash":2.0,
    # Bangalore
    "indiranagar":1.9,    "koramangala":1.8,     "whitefield":1.4,
    "hsr layout":1.6,     "jayanagar":1.5,       "electronic city":1.0,
    "sarjapur":1.2,       "marathahalli":1.3,
    # Hyderabad
    "jubilee hills":2.2,  "banjara hills":2.0,   "hitech city":1.6,
    "gachibowli":1.5,     "kondapur":1.4,
    # Pune
    "koregaon park":1.8,  "kalyani nagar":1.6,   "wakad":1.2,
    "hinjewadi":1.3,      "kharadi":1.3,
    # Chennai
    "anna nagar":1.7,     "t nagar":1.6,         "adyar":1.8,
    "velachery":1.3,      "nungambakkam":1.9,
    # Gurugram/NCR
    "dlf":1.8,            "golf course":2.1,     "sohna road":1.1,
    "sector 57":1.3,      "sector 47":1.4,
    # Muzaffarnagar areas
    "begrajpur":1.2,      "shahpur":1.1,         "jansath road":1.0,
    "civil lines":1.3,    "budhana road":0.9,    "ghanta ghar":1.1,
    # Meerut areas
    "shastri nagar":1.3,  "ganga nagar":1.2,     "cantonment":1.4,
    "abupura":1.1,
}

FEATURE_PREMIUMS = {
    "balcony":0.025,  "terrace":0.04,   "storeroom":0.015,
    "pooja_room":0.015,"servant_qr":0.035,"parking":0.03,
    "lift":0.025,     "gym":0.015,      "pool":0.04,
    "garden":0.03,    "security":0.015,
    "furnished":0.10, "semi_furnished":0.05,
}

PROPERTY_TYPE_MULT = {
    "Independent House / Villa":1.20,
    "Apartment / Flat":1.00,
    "Builder Floor":1.05,
    "Penthouse":1.35,
    "Studio Apartment":0.92,
    "Row House":1.12,
    "Duplex":1.15,
    "Commercial Property":1.30,
    "Agricultural / Plot":0.40,
}

AGE_DISCOUNT = {
    "New / Under Construction":1.05,
    "0-2 years":1.00,
    "3-5 years":0.95,
    "6-10 years":0.88,
    "11-20 years":0.78,
...
# ══ CITY GROWTH RATES (Annual CAGR — realistic 2024 data) ══
CITY_GROWTH_RATES = {
    # Tier 1 — high growth
    "hyderabad":11.5,   "bengaluru":10.8,   "bangalore":10.8,
    "pune":9.5,         "ahmedabad":9.0,    "mumbai":8.5,
    "delhi":7.8,        "gurugram":9.2,     "gurgaon":9.2,
    "noida":8.8,        "chennai":7.5,      "kolkata":6.8,
    # Tier 2 — moderate growth
    "indore":9.5,       "dehradun":8.8,     "kochi":8.0,
    "chandigarh":7.5,   "lucknow":7.2,      "ghaziabad":7.8,
    "jaipur":7.0,       "surat":7.5,        "nagpur":7.0,
    "bhubaneswar":7.5,  "guwahati":7.0,     "coimbatore":6.8,
    # Tier 3 UP / Small cities — slow growth
    "muzaffarnagar":5.5,"meerut":6.0,       "saharanpur":5.0,
    "bareilly":5.5,     "moradabad":5.0,    "aligarh":5.2,
    "mathura":5.8,      "agra":6.0,         "varanasi":6.2,
    "prayagraj":5.8,    "gorakhpur":5.5,    "kanpur":5.5,
    # Default
    "default":5.5,
}
...
CITY_ALIASES = {
    ...
    "delhi":"delhi","new delhi":"new delhi","national capital territory of delhi":"delhi",
    "hyderabad":"hyderabad","secunderabad":"hyderabad","rachakonda":"hyderabad",
    "pune city":"pune","pimpri-chinchwad":"pune","pcmc":"pune",
    "chennai":"chennai","madras":"chennai","greater chennai":"chennai",
    "kolkata":"kolkata","calcutta":"kolkata","kolkata municipal":"kolkata",
    ...
    "dehradun":"dehradun","haridwar":"haridwar","rishikesh":"haridwar",
    "coimbatore":"coimbatore","madurai":"madurai","trichy":"trichy",
    "mysuru":"mysuru","mysore":"mysuru","mangaluru":"mangaluru","mangalore":"mangaluru",
...
def geocode_address(address_str):
    try:
        geo = Nominatim(user_agent="gharmool_accurate_v3")
...
def reverse_geocode(lat, lon):
    try:
        geo = Nominatim(user_agent="gharmool_rev_v3")
...
def get_per_sqft_rate(city, area):
    ...
    # ✅ BUG FIX: Only apply area multiplier if area is non-empty AND explicitly matches
    area_mult = 1.0
    if area_l:  # <-- CRITICAL FIX: skip if area is empty string
        for ak, mult in AREA_MULTIPLIERS.items():
            # Require meaningful match — area key must be at least 4 chars
...
# ══════════════════════════════════════════════════════════════
# VALUATION ENGINE (Calibrated Formula)
# ══════════════════════════════════════════════════════════════
def calculate_valuation(sqft, bedrooms, bathrooms, toilets, balconies, terrace,
    ...
    feats = {
        "balcony":   bool(balconies),
        "terrace":   terrace,
        "storeroom": storeroom,
        "pooja_room":pooja_room,
        "servant_qr":servant_qr,
        "parking":   parking,
        "lift":      lift,
        "gym":       gym,
        "pool":      pool,
    ...
    # Balcony count bonus (max 2 balconies count)
    ...
    # Room premium (calibrated — not too aggressive)
    room_bonus = (min(bedrooms, 6) * 0.03) + (bathrooms * 0.01) + (toilets * 0.008)

    # Cap total multiplier to avoid unrealistic values
    feat_room_total = min(feat_total + room_bonus, 0.45)  # max 45% premium from features

    final_mult = type_mult * age_mult * floor_mult * (1 + feat_room_total)
    val = int(base_val * final_mult)
...
def build_map(lat, lon, address="Property"):
    ...
    folium.Marker([lat, lon],
        ...
        popup=folium.Popup(f"""<div style='font-family:sans-serif;width:220px;'>
            <b style='color:#f5a623;'>📍 Property</b><br><small>{address}</small><br><br>
...
def generate_floor_plan(bedrooms, bathrooms, toilets, balconies, terrace,
    ...
    rooms = [
        ("LIVING ROOM","🛋",2,22,28,24,C["liv"],C["liv_bd"]),
    ...
    ax.text(50,1.5,f"Total Width ~{int(math.sqrt(sqft*1.25))} ft",fontsize=5,color=C["dim"],ha="center")
    ...
    ax.text(50,68.5,f"Area:{sqft} sqft | {bedrooms}BHK | {bathrooms}Bath | {balconies}Balcony | {datetime.date.today()}",
    ...
    ax.legend(handles=[mpatches.Patch(color=C["bed_bd"],label="Bedrooms"),
              ...
              loc="lower right",fontsize=6,fancybox=True,facecolor="#0e1219",
              edgecolor=C["wall"],labelcolor="white",framealpha=0.9)
...
# ══════════════════════════════════════════════════════════════
# FORECAST ENGINE (Realistic Tier-aware)
# ══════════════════════════════════════════════════════════════
def get_city_tier(city):
    tier1 = ["mumbai","delhi","bengaluru","bangalore","hyderabad","pune","chennai","kolkata","gurugram","gurgaon","noida"]
    tier2 = ["ahmedabad","jaipur","lucknow","surat","indore","chandigarh","kochi","dehradun","bhubaneswar","guwahati","nagpur"]
...
def generate_forecast(val, city, years=15):
    ...
    # Development-adjusted rate
    dev_score = (idx["metro"]+idx["it_hub"]+idx["infra"]+idx["startup"])/400.0
    adj = rate * (0.8 + dev_score * 0.4)

    # Tier 3 cities: cap at 8% max growth
    if tier == 3: adj = min(adj, 7.5)
    ...
    for i,yr in enumerate(yrs):
        if yr in [yrs[0]+5, yrs[0]+10, yrs[-1]]:
            fig.add_annotation(x=yr,y=base[i],
                text=f"Rs.{base[i]/1e7:.2f}Cr" if base[i]<1e7 else f"Rs.{base[i]/1e7:.1f}Cr",
...
with tab1:
    ...
    with col_inp:
        ...
        method = st.radio("Search Mode",
                          ["📝 Address / Locality","📌 Lat / Lon Coordinates"],
                          horizontal=True, key="search_mode")

        if method == "📝 Address / Locality":
            addr_in = st.text_input(
                "Enter Address, Locality or Pincode",
                placeholder="e.g. Civil Lines, Muzaffarnagar  or  Indiranagar, Bangalore",
                key="addr_search_input"
            )
            search_clicked = st.button("🔍 Find on Map", use_container_width=True, key="find_map_btn")
            if search_clicked:
                if addr_in.strip():
                    ...
                    if res:
                        ...
                        st.error("❌ Location not found. Try adding city name. e.g. 'Civil Lines, Muzaffarnagar'")
                else:
                    st.warning("⚠️ Please enter an address first.")
        else:
            c1,c2 = st.columns(2)
            lat_in = c1.number_input("Latitude",  value=28.3909, format="%.5f", key="lat_input")
            lon_in = c2.number_input("Longitude", value=77.7061, format="%.5f", key="lon_input")
            if st.button("📌 Fetch Address", use_container_width=True, key="rev_geo_btn"):
        ...
        if st.session_state.geo_result:
            ...
            tier_label = ["🥇 Tier 1","🥈 Tier 2","🥉 Tier 3"][get_city_tier(st.session_state.city_name)-1]

            st.markdown(f"""
            <div class="gcard" style="border-color:rgba(0,198,167,0.3);">
              <div class="gcard-title">📋 Detected Address</div>
              <div class="info-row"><span class="info-label">📍 Area/Locality</span>
              ...
              <div class="info-row"><span class="info-label">🏆 City Tier</span>
                <span class="info-val">{tier_label}</span></div>
              <div style="margin-top:0.8rem;border-top:1px solid rgba(245,166,35,0.15);padding-top:0.8rem;">
                <div style="color:#7a8499;font-size:0.78rem;margin-bottom:0.3rem;">ESTIMATED MARKET RATE (2024)</div>
                <div style="color:#f5a623;font-weight:800;font-size:1.4rem;">Rs.{st.session_state.per_sqft:,} / sq ft</div>
              </div>
            </div>
...
with tab2:
    if not st.session_state.geo_result:
        ...
        tier_lbl = ["🥇 Tier 1 City","🥈 Tier 2 City","🥉 Tier 3 City"][get_city_tier(st.session_state.city_name)-1]
        st.markdown(f"""<p style='color:#7a8499;'>Market rate: 
            <strong style='color:#f5a623;'>Rs.{st.session_state.per_sqft:,}/sqft</strong> &nbsp;|&nbsp; 
            {tier_lbl}</p>""", unsafe_allow_html=True)

        col_form,col_result = st.columns([1.3,1.7])

        with col_form:
            with st.form("property_form_v2"):
                st.markdown("**📐 Area & Type**")
                c1,c2 = st.columns(2)
                sqft      = c1.number_input("Total Area (sq ft)",100,50000,1200,50,key="v_sqft")
                prop_name = c2.text_input("Property Name","My Home",key="v_pname")
                prop_type = st.selectbox("Property Type",list(PROPERTY_TYPE_MULT.keys()),key="v_ptype")
                c1,c2 = st.columns(2)
                age   = c1.selectbox("Property Age",list(AGE_DISCOUNT.keys()),key="v_age")
                floor = c2.selectbox("Floor Level",list(FLOOR_MULT.keys()),key="v_floor")

                st.markdown("**🛏️ Rooms**")
                c1,c2,c3 = st.columns(3)
                bedrooms  = c1.number_input("Bedrooms",0,20,3,1,key="v_bed")
                bathrooms = c2.number_input("Bathrooms",0,15,2,1,key="v_bath")
                toilets   = c3.number_input("Toilets",0,15,1,1,key="v_toilet")

                st.markdown("**🌿 Extra Spaces**")
                c1,c2,c3 = st.columns(3)
                balconies  = c1.number_input("Balconies",0,10,1,1,key="v_bal")
                terrace    = c2.checkbox("Terrace",key="v_terrace")
                storeroom  = c3.checkbox("Store Room",key="v_store")
                c1,c2,c3 = st.columns(3)
                pooja_room = c1.checkbox("Pooja Room",key="v_pooja")
                servant_qr = c2.checkbox("Servant Qtr.",key="v_servant")
                garden     = c3.checkbox("Garden/Lawn",key="v_garden")

                st.markdown("**🏢 Amenities**")
                c1,c2,c3 = st.columns(3)
                parking  = c1.checkbox("Parking",value=True,key="v_parking")
                lift     = c2.checkbox("Lift/Elevator",key="v_lift")
                gym      = c3.checkbox("Gym",key="v_gym")
                c1,c2 = st.columns(2)
                pool     = c1.checkbox("Swimming Pool",key="v_pool")
                security = c2.checkbox("24x7 Security",value=True,key="v_security")

                furnishing = st.selectbox("Furnishing Status",
                    ["Unfurnished","Semi Furnished","Fully Furnished"],key="v_furnish")

                st.markdown("---")
                rate_override = st.number_input(
                    "🔧 Override Rate (Rs./sqft) — Leave 0 for auto",
                    0,500000,0,100,key="v_rate",
                    help="Enter 0 to use the auto-detected location-based rate")
        ...
        with col_result:
            if submitted:
                ...
                fig_bd.update_layout(title="💡 Valuation Breakdown",
                    ...
                    xaxis=dict(showgrid=False,color="#7a8499",tickangle=-25),
                ...
                st.markdown(f"""
                <div class="gcard" style="border-color:rgba(0,198,167,0.3);">
                  <div class="gcard-title">🏷️ Rental & Yield Estimates</div>
                  <div class="info-row"><span class="info-label">Estimated Monthly Rent</span>
                    <span class="info-val" style="color:#00c6a7;">Rs.{mr:,}</span></div>
                  <div class="info-row"><span class="info-label">Annual Rental Yield</span>
                    <span class="info-val" style="color:#00c6a7;">{ay}%</span></div>
                  <div class="info-row"><span class="info-label">Annual Rent Income</span>
                    <span class="info-val">Rs.{mr*12:,}</span></div>
                  <div class="info-row"><span class="info-label">Break-even Period</span>
                ...
                st.markdown(f"""<div class="val-banner">
                  <div style="font-size:0.8rem;color:#7a8499;">LAST CALCULATED VALUE</div>
                  <div class="val-price">Rs.{v:,}</div>
                  <div style="font-size:1rem;color:#e8eaf0;">~ {v/1e7:.2f} Crore</div></div>""",
                    unsafe_allow_html=True)
                st.info("Fill form and click **Calculate Valuation** to refresh.")
            else:
                st.info("👈 Fill the property details on the left and click **Calculate Valuation**.")
...
with tab3:
    ...
    with col_fp1:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
        st.markdown("""<div class="gcard" style="font-size:0.8rem;color:#7a8499;">
          <div class="gcard-title">📖 Legend</div>
          🔵 Bedrooms &nbsp; 🟢 Living/Dining<br>🟣 Kitchen &nbsp; 🩵 Baths<br>🧭 N = North
        </div>""", unsafe_allow_html=True)
    ...
    with col_fp2:
        ...
        if st.session_state.get("fp_buf"):
            ...
            st.download_button("⬇️ Download Floor Plan (PNG)",data=img,
            ...
            st.markdown("""<div class="gcard" style="border-color:rgba(0,198,167,0.3);margin-top:1rem;">
              ...
              <div style="font-size:0.85rem;color:#e8eaf0;line-height:1.8;">
                🌞 <strong>Orientation:</strong> Living room East-facing for morning light.<br>
                🌬️ <strong>Ventilation:</strong> Windows on opposite walls for cross-ventilation.<br>
                🚿 <strong>Wet Areas:</strong> Group bathrooms together — saves plumbing cost.<br>
                ...
                💡 <strong>Vastu:</strong> Master bed SW, Kitchen SE, Pooja Room NE.
...
with tab4:
    ...
    val_fc  = st.session_state.valuation if st.session_state.valuation > 0 else 3000000
    col_fc1,col_fc2 = st.columns([1,2.5])

    with col_fc1:
        st.markdown('<div class="gcard">', unsafe_allow_html=True)
        st.markdown('<div class="gcard-title">⚙️ Forecast Settings</div>', unsafe_allow_html=True)
        fc_val   = st.number_input("Current Property Value (Rs.)",
                                   100000,1000000000,max(val_fc,500000),50000,key="fc_val")
        fc_city  = st.text_input("City",city_fc.title() or "Muzaffarnagar",key="fc_city")
        fc_years = st.slider("Forecast Years",5,15,15,key="fc_years")
        st.markdown("</div>", unsafe_allow_html=True)

        gr2 = CITY_GROWTH_RATES.get(fc_city.lower().strip(), CITY_GROWTH_RATES["default"])
        tier_fc = get_city_tier(fc_city.lower().strip())
        tier_labels = ["🥇 Tier 1 (High Growth)","🥈 Tier 2 (Moderate)","🥉 Tier 3 (Steady)"]
        st.markdown(f"""<div class="gcard" style="border-color:rgba(0,198,167,0.3);">
          <div class="gcard-title">📊 City Growth Profile</div>
          <div class="info-row"><span class="info-label">City Tier</span>
            <span class="info-val">{tier_labels[tier_fc-1]}</span></div>
          <div class="info-row"><span class="info-label">Historical CAGR</span>
            <span class="info-val" style="color:#00c6a7;">{gr2:.1f}% / year</span></div>
          <div class="info-row"><span class="info-label">5yr Projection</span>
            <span class="info-val">Rs.{int(fc_val*(1+gr2/100)**5):,}</span></div>
          <div class="info-row"><span class="info-label">10yr Projection</span>
            <span class="info-val">Rs.{int(fc_val*(1+gr2/100)**10):,}</span></div>
          <div class="info-row"><span class="info-label">15yr Projection</span>
...
with tab5:
    ...
    with col_mi1:
        ...
        fig_cmp.add_hline(y=st.session_state.per_sqft if st.session_state.per_sqft>0 else 2500,
            line_dash="dot",line_color="#f5a623",
            annotation_text=f"Your City ({city_mi.title()})",annotation_font_color="#f5a623")
        fig_cmp.update_layout(title="💹 Base Rate Comparison (Rs./sqft)",
            paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
    ...
    with col_mi2:
        ...
        # ✅ FIXED colorbar (no titlefont deprecated key)
        ...
        fig_hm.update_layout(
            ...
            paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#e8eaf0"),
            xaxis=dict(color="#7a8499",tickangle=-20),
            yaxis=dict(color="#7a8499"),
            height=320,margin=dict(t=50,b=60),title_font=dict(color="#f5a623"))
        st.plotly_chart(fig_hm, use_container_width=True)

        cities_gr = ["bengaluru","hyderabad","pune","mumbai","delhi","meerut",
                     "muzaffarnagar","lucknow","agra","varanasi"]
        gr_vals   = [CITY_GROWTH_RATES.get(c,5.5) for c in cities_gr]
        fig_gr = go.Figure(go.Bar(
            x=[c.title() for c in cities_gr], y=gr_vals,
            marker=dict(color=gr_vals,colorscale=[[0,"#ff4d6d"],[0.5,"#f5a623"],[1,"#06d6a0"]]),
            text=[f"{v:.1f}%" for v in gr_vals],
...
st.markdown("""<div style="text-align:center;color:#7a8499;font-size:0.82rem;padding:1rem;">
  ...
  <em>Valuations are indicative estimates — consult a registered valuer for official assessment</em>
</div>""", unsafe_allow_html=True)
