...
CITY_BASE_RATES = {
    ...
    "greater noida":4800,"faridabad":5500,"ahmedabad":4500,
    "jaipur":4200,"lucknow":4000,"surat":4800,"kanpur":3200,
    "nagpur":3800,"indore":4500,"bhopal":3800,"vadodara":4000,
    "coimbatore":4200,"vizag":4000,"visakhapatnam":4000,
    "patna":3500,"ranchi":3200,"raipur":3000,"kochi":6000,
    "cochin":6000,"thiruvananthapuram":5000,"chandigarh":6500,
    "ludhiana":4000,"amritsar":3800,"agra":3200,"varanasi":3500,
    "meerut":3200,"nashik":4200,"aurangabad":3500,"solapur":3000,
    "madurai":3800,"trichy":3500,"salem":3200,"mysuru":5000,
    "mysore":5000,"hubli":3500,"mangalore":5500,"mangaluru":5500,
    "bhubaneswar":4200,"guwahati":4000,"dehradun":5500,"shimla":6000,
    "jodhpur":3500,"udaipur":3800,"ajmer":3000,"allahabad":3200,
    "prayagraj":3200,"ghaziabad":5500,"muzaffarnagar":2800,
    "meerut":3200,"saharanpur":2500,"bareilly":2800,"moradabad":2500,
    "aligarh":2600,"mathura":2800,"firozabad":2200,"etawah":2000,
...
FEATURE_PREMIUMS = {
    ...
    "pool":0.05,"garden":0.04,"security":0.02,
    "furnished":0.12,"semi_furnished":0.06,
}

PROPERTY_TYPE_MULT = {
    "Independent House / Villa":1.25,"Apartment / Flat":1.00,
    "Builder Floor":1.08,"Penthouse":1.40,"Studio Apartment":0.90,
    "Row House":1.15,"Duplex":1.20,"Commercial Property":1.35,
    "Agricultural / Plot":0.45,
}
...
CITY_GROWTH_RATES = {
    ...
    "indore":11.0,"kochi":9.0,"chandigarh":8.5,"dehradun":10.2,
    "default":7.5,
}
...
CITY_ALIASES = {
    ...
    "lucknow":"lucknow","indore":"indore",
    "chandigarh":"chandigarh","kochi":"kochi","cochin":"kochi",
    "dehradun":"dehradun","muzaffarnagar":"muzaffarnagar",
    "meerut":"meerut","ghaziabad":"ghaziabad","saharanpur":"saharanpur",
}
...
def geocode_address(address_str):
    try:
        geolocator = Nominatim(user_agent="gharmool_v2_app")
        location = geolocator.geocode(address_str + ", India", timeout=10, language="en")
        if location:
...
def reverse_geocode(lat, lon):
    try:
        geolocator = Nominatim(user_agent="gharmool_v2_rev")
...
def extract_city(raw_data):
    ...
    for key in ["city","town","city_district","municipality","county","state_district"]:
        val = addr.get(key, "")
        if val:
            v = val.lower().strip()
    ...
    for known in CITY_ALIASES:
        if known in display.lower():
            return CITY_ALIASES[known]
    return ""

def extract_area(raw_data):
    addr = raw_data.get("address", {}) if raw_data else {}
    for key in ["neighbourhood","suburb","quarter","residential","road"]:
        val = addr.get(key, "")
        if val: return val.lower().strip()
    return ""

def extract_state(raw_data):
    return (raw_data.get("address", {}) if raw_data else {}).get("state", "")

def extract_pincode(raw_data):
    pc = (raw_data.get("address", {}) if raw_data else {}).get("postcode", "")
    return pc if pc else "N/A"

def get_per_sqft_rate(city, area):
    base = CITY_BASE_RATES.get(city.lower().strip(), CITY_BASE_RATES["default_urban"])
    area_mult = 1.0
    for ak, mult in AREA_MULTIPLIERS.items():
        if ak in area.lower() or area.lower() in ak:
            area_mult = mult; break
...
def calculate_valuation(sqft, bedrooms, bathrooms, toilets, balconies, terrace,
    ...
    feats = {"balcony":balconies,"terrace":terrace,"storeroom":storeroom,
    ...
    val = int(base_val * type_mult * age_mult * floor_mult * (1 + feat_total + room_bonus))
    breakdown = {
...
def build_map(lat, lon, address="Property"):
    ...
    folium.Marker([lat,lon],
        ...
        popup=folium.Popup(f"""<div style='font-family:sans-serif;width:220px;'>
            ...
            Lat: {lat:.5f}<br>Lon: {lon:.5f}</div>""", max_width=250),
...
def generate_floor_plan(bedrooms, bathrooms, toilets, balconies, terrace,
    ...
    misc = []
    if storeroom: misc.append(("STORE ROOM","📦","#1a1008","#a78bfa"))
    if pooja_room: misc.append(("POOJA ROOM","🪔","#1a0808","#f97316"))
    if servant_qr: misc.append(("SERVANT QR","🏠","#0a0a0a","#888"))
    ...
    ax.annotate("",xy=(98,0.5),xytext=(2,0.5),
                arrowprops=dict(arrowstyle="<->",color=C["dim"],lw=0.8))
    ax.text(50,1.5,f"Total Width ~{int(math.sqrt(sqft*1.25))} ft",
            fontsize=5,color=C["dim"],ha="center")
    ax.annotate("",xy=(95,67),xytext=(95,63),
                arrowprops=dict(arrowstyle="->",color=C["wall"],lw=2))
    ax.text(95,68,"N",ha="center",va="center",color=C["wall"],fontsize=10,fontweight="bold")
    ax.plot([3,13],[1.2,1.2],color="#888",lw=2)
    ax.plot([3,3],[0.9,1.5],color="#888",lw=1.5)
    ax.plot([13,13],[0.9,1.5],color="#888",lw=1.5)
    ax.text(8,0.3,"Scale",ha="center",fontsize=5,color="#888")
    ax.text(50,69.5,f"GHARMOOL FLOOR PLAN  -  {prop_name.upper()}",
            ha="center",va="center",fontsize=9,fontweight="bold",
            color=C["wall"],fontfamily="monospace")
    ax.text(50,68.5,
            f"Area:{sqft} sqft | {bedrooms}BHK | {bathrooms}Baths | {balconies}Balcony | {datetime.date.today()}",
            ha="center",va="center",fontsize=6,color="#888",fontfamily="monospace")
    ax.legend(handles=[
        mpatches.Patch(color=C["bed_bd"],label="Bedrooms"),
        mpatches.Patch(color=C["bath_bd"],label="Baths"),
        mpatches.Patch(color=C["liv_bd"],label="Living/Dining"),
    ...
    fig.savefig(buf,format="png",dpi=160,bbox_inches="tight",
                facecolor="#0e1219",edgecolor="none")
...
def generate_forecast(val, city, years=15):
    ...
    boost = (idx["metro"]+idx["it_hub"]+idx["infra"]+idx["startup"])/400
    adj   = rate * (0.7 + boost*0.6)
    now   = datetime.date.today().year
    yrs   = list(range(now, now+years+1))
    return (yrs,
            [val*((1+adj*1.35/100)**i) for i in range(years+1)],
            [val*((1+adj/100)**i)      for i in range(years+1)],
            [val*((1+adj*0.65/100)**i) for i in range(years+1)],
            adj)
...
def forecast_chart(yrs, opt, base, cons, val, city):
    ...
    for i,yr in enumerate(yrs):
        if yr in [yrs[0]+5, yrs[0]+10, yrs[-1]]:
            fig.add_annotation(x=yr,y=base[i],text=f"Rs.{base[i]/1e7:.1f}Cr",
                showarrow=True,arrowhead=2,arrowcolor="#f5a623",
                font=dict(color="#f5a623",size=11),
                bgcolor="rgba(14,18,25,0.9)",bordercolor="#f5a623",
...
with tab1:
    ...
    with col_inp:
        ...
        if method == "📝 Address / Locality":
            addr_in = st.text_input("Enter Address",placeholder="e.g. Indiranagar, Bangalore")
        ...
        if st.session_state.geo_result:
            ...
            <div class="gcard" style="border-color:rgba(0,198,167,0.3);">
              ...
              <div class="info-row"><span class="info-label">📮 PIN</span><span class="info-val">{st.session_state.pincode}</span></div>
              <div class="info-row"><span class="info-label">🌐 Lat/Lon</span><span class="info-val">{lat_g:.4f}, {lon_g:.4f}</span></div>
              <div class="info-row" style="margin-top:0.8rem;border-top:1px solid rgba(245,166,35,0.15);padding-top:0.8rem;">
                ...
                <span style="color:#f5a623;font-weight:800;font-size:1.1rem;">Rs.{st.session_state.per_sqft:,}/sqft</span>
              </div>
            </div>
            <div class="gcard" style="padding:1rem;">
              ...
              <div style="display:flex;gap:0.6rem;flex-wrap:wrap;">
                <a href="https://www.google.com/maps/search/?api=1&query={gmaps_q}" target="_blank"
                   style="background:rgba(66,133,244,0.15);border:1px solid #4285f4;color:#4285f4;padding:6px 14px;border-radius:8px;font-size:0.82rem;text-decoration:none;">🗺️ Maps</a>
                <a href="https://www.google.com/maps/@?api=1&map_action=pano&viewpoint={lat_g},{lon_g}" target="_blank"
                   style="background:rgba(234,67,53,0.15);border:1px solid #ea4335;color:#ea4335;padding:6px 14px;border-radius:8px;font-size:0.82rem;text-decoration:none;">🚶 Street View</a>
                <a href="https://www.google.com/maps/search/schools+near/@{lat_g},{lon_g},15z" target="_blank"
                   style="background:rgba(0,198,167,0.15);border:1px solid #00c6a7;color:#00c6a7;padding:6px 14px;border-radius:8px;font-size:0.82rem;text-decoration:none;">🏫 Nearby</a>
                <a href="https://www.google.com/maps/dir/?api=1&destination={lat_g},{lon_g}" target="_blank"
                   style="background:rgba(245,166,35,0.15);border:1px solid #f5a623;color:#f5a623;padding:6px 14px;border-radius:8px;font-size:0.82rem;text-decoration:none;">🧭 Directions</a>
              </div>
    ...
    with col_map:
        if st.session_state.geo_result:
            ...
            st_folium(m0,width="100%",height=500,returned_objects=[])
            st.markdown("</div>", unsafe_allow_html=True)
...
with tab2:
    if not st.session_state.geo_result:
        st.warning("⚠️ Go to 📍 Location & Map tab first and search your address.")
    else:
            ...
            if submitted:
                ...
                <div class="val-banner">
                  ...
                  <div style="font-size:0.85rem;color:#7a8499;margin-top:0.5rem;">
                    📍 {st.session_state.area_name.title()}, {st.session_state.city_name.title()} · Rs.{eff:,}/sqft effective
                  </div>
                </div>""", unsafe_allow_html=True)

                st.markdown("<br>", unsafe_allow_html=True)
                c1,c2,c3,c4 = st.columns(4)
                for col,ico,vk,lk in [(c1,"📐",f"{sqft:,}","Area(sqft)"),(c2,"💰",f"Rs.{use_rate:,}","Rate/sqft"),(c3,"🛏",f"{bedrooms}BHK","Config"),(c4,"📅",age.split()[0],"Age")]:
                    col.markdown(f'<div class="kpi"><div class="kpi-icon">{ico}</div><div class="kpi-val">{vk}</div><div class="kpi-lab">{lk}</div></div>',unsafe_allow_html=True)
                ...
                <div class="gcard" style="border-color:rgba(0,198,167,0.3);">
                  ...
                  <div class="info-row"><span class="info-label">Monthly Rent</span><span class="info-val" style="color:#00c6a7;">Rs.{mr:,}</span></div>
                  <div class="info-row"><span class="info-label">Annual Yield</span><span class="info-val" style="color:#00c6a7;">{ay}%</span></div>
                  <div class="info-row"><span class="info-label">Annual Income</span><span class="info-val">Rs.{mr*12:,}</span></div>
                  <div class="info-row"><span class="info-label">Break-even</span><span class="info-val">~{int(val/(mr*12))} years</span></div>
                </div>""", unsafe_allow_html=True)

            elif st.session_state.valuation>0:
                v=st.session_state.valuation
                st.markdown(f'<div class="val-banner"><div style="font-size:0.85rem;color:#7a8499;">LAST VALUE</div><div class="val-price">Rs.{v:,}</div><div style="color:#e8eaf0;">~ {v/1e7:.2f} Crore</div></div>',unsafe_allow_html=True)
...
with tab4:
    ...
    city_fc = st.session_state.city_name or "bengaluru"
    val_fc  = st.session_state.valuation  if st.session_state.valuation>0 else 8000000
    col_fc1,col_fc2 = st.columns([1,2.5])

    with col_fc1:
        st.markdown('<div class="gcard">', unsafe_allow_html=True)
        st.markdown('<div class="gcard-title">⚙️ Forecast Settings</div>', unsafe_allow_html=True)
        fc_val   = st.number_input("Current Value (Rs.)",100000,1000000000,max(val_fc,2000000),100000)
        fc_city  = st.text_input("City",city_fc.title() or "Bangalore")
        ...
        st.markdown(f"""<div class="gcard" style="border-color:rgba(0,198,167,0.3);">
          ...
          <div class="info-row"><span class="info-label">Avg CAGR</span><span class="info-val" style="color:#00c6a7;">{gr2:.1f}%</span></div>
          <div class="info-row"><span class="info-label">Tier</span><span class="info-val">{"Tier 1" if gr2>=9 else "Tier 2" if gr2>=7.5 else "Tier 3"}</span></div>
          <div class="info-row"><span class="info-label">10yr Projection</span><span class="info-val">Rs.{int(fc_val*(1+gr2/100)**10):,}</span></div>
        </div>""", unsafe_allow_html=True)

    with col_fc2:
        yrs,opt,base,cons,_ = generate_forecast(fc_val, fc_city.strip(), fc_years)
        st.plotly_chart(forecast_chart(yrs,opt,base,cons,fc_val,fc_city), use_container_width=True)
        rows=[]
        for i,yr in enumerate(yrs):
            if i%3==0 or i==len(yrs)-1:
                rows.append({"Year":yr,"Conservative":f"Rs.{int(cons[i]):,}",
                             "Base Case":f"Rs.{int(base[i]):,}","Optimistic":f"Rs.{int(opt[i]):,}",
...
with tab5:
    ...
    city_mi = st.session_state.city_name or "bengaluru"
    ...
    with col_mi1:
        ...
        fig_cmp.add_hline(y=st.session_state.per_sqft if st.session_state.per_sqft>0 else 8500,
            line_dash="dot",line_color="#f5a623",annotation_text="Your Location",
            annotation_font_color="#f5a623")
        fig_cmp.update_layout(title="💹 Base Rate by City (Rs./sqft)",
            paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#e8eaf0"),
    ...
    with col_mi2:
        ...
        # ✅ FIXED: titlefont -> title=dict(text=..., font=dict(...))
        fig_hm = go.Figure(go.Heatmap(
            z=z_data,
            x=[k.split("/")[0].strip()[:18] for k in ptype_keys],
            y=area_keys,
            ...
            colorbar=dict(
                title=dict(text="Lakhs", font=dict(color="#e8eaf0")),
                thickness=12,
